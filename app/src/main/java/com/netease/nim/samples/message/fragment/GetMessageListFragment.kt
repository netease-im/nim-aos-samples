...
class GetMessageListFragment : BaseMethodExecuteFragment<FragmentGetMessageListBinding>() {
    /**
     * 锚点消息
     */
    ...
    override fun onRequest() {
        ...
        activityViewModel.showLoadingDialog()
        NIMClient.getService(V2NIMMessageService::class.java).getMessageList(option,
    ...
    private fun updateResult(messageList: List<V2NIMMessage>?, error: V2NIMError?) {
        ...
        if (messageList != null) {
            if (messageList.isEmpty()) {
                activityViewModel.refresh("查询成功，共 0 条消息")
            } else {
                activityViewModel.refresh(
                    "查询成功，共 ${messageList.size} 条消息,result = +\n" + getV2NIMMessageListJson(
                        messageList
                    )
                )
            }
        }else{
            activityViewModel.refresh("查询成功，共 0 条消息,result = null")
        }
    }
