...
import com.netease.nim.samples.databinding.FragmentRemoveUserListenerBinding
import com.netease.nim.samples.user.common.GlobalUserListenerManager

class RemoveUserListenerFragment : BaseMethodExecuteFragment<FragmentRemoveUserListenerBinding>() {
    ...
    override fun createViewBinding(
        ...
        container: ViewGroup?
    ): FragmentRemoveUserListenerBinding {
        return FragmentRemoveUserListenerBinding.inflate(inflater, container, false)
    }
    ...
    private fun removeUserListener() {
        ...
        // 执行移除操作
        val success = GlobalUserListenerManager.removeListener()

        if (success) {
            updateOperationResult("✅ 监听器移除成功! 已停止接收所有用户资料事件回调")
            showToast("✅ 监听器移除成功")
        } else {
    ...
    override fun onRequest() {
        ...
        val isListenerActive = GlobalUserListenerManager.isListenerAdded()
        ...
        val statusText = buildString {
            ...
            if (isListenerActive) {
                ...
                append("✅ 发现活跃监听器:\n")
                append("• 类型: V2NIMUserListener\n")
                append("• 状态: 全局监听中\n")
                append("• 可以执行移除操作\n\n")

                append("执行移除:\n")
                append("• removeUserListener: 移除指定监听器\n")
                append("• 移除后立即停止接收所有回调事件\n\n")
                
                append("停止的监听事件:\n")
                append("• onUserProfileChanged: 用户资料变更通知\n")
                append("• onBlockListAdded: 黑名单添加通知\n")
                append("• onBlockListRemoved: 黑名单移除通知\n\n")

            } else {
                append("ℹ️ 当前状态:\n")
                append("• 没有活跃的用户资料监听器\n")
                append("• 无需执行移除操作\n")
                append("• 如需监听用户事件，请先添加监听器\n\n")
