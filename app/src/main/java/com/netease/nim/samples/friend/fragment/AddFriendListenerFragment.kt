...
import com.netease.nim.samples.databinding.FragmentAddFriendListenerBinding
import com.netease.nim.samples.user.common.GlobalFriendListenerManager
import java.text.SimpleDateFormat
import java.util.*

class AddFriendListenerFragment : BaseMethodExecuteFragment<FragmentAddFriendListenerBinding>() {
    ...
    override fun createViewBinding(
        ...
        container: ViewGroup?
    ): FragmentAddFriendListenerBinding {
        return FragmentAddFriendListenerBinding.inflate(inflater, container, false)
    }
    ...
    private fun addUserListener() {
        ...
        val success = GlobalFriendListenerManager.addListener(requireContext())
        if (success) {
    ...
    private fun clearHistory() {
        GlobalFriendListenerManager.clearHistory()
        updateUI()
        showToast("已清空所有历史记录")
    }
    ...
    private fun updateListenerStatus() {
        ...
        // 更新最新回调显示
        val latestCallback = GlobalFriendListenerManager.getLatestCallback()
    ...
    private fun updateStatistics() {
        val statistics = GlobalFriendListenerManager.getStatistics()
        val statisticsText = "总回调次数: ${statistics.totalCallbackCount}\n" +
                "好友资料变更: ${statistics.friendProfileChangeCount} | 好友添加: ${statistics
                    .friendAddedCount} | 好友移除: ${statistics.friendRemovedCount}"
    ...
    private fun updateHistoryDisplay() {
        val history = GlobalFriendListenerManager.getCallbackHistory()
    ...
    private fun updateListenTime() {
        val listenStartTime = GlobalFriendListenerManager.getListenStartTime()
        if (GlobalFriendListenerManager.isListenerAdded() && listenStartTime > 0) {
    ...
    override fun onRequest() {
        ...
        val statusText = buildString {
            append("全局用户监听器状态:\n")
            append("是否已添加: ${if (GlobalFriendListenerManager.isListenerAdded()) "是" else "否"}\n")
            ...
            append("历史记录数量: ${GlobalFriendListenerManager.getCallbackHistory().size}\n\n")

            append("监听器回调事件说明:\n")
            append("• onFriendInfoChanged: 好友信息变更\n")
            append("• onFriendAdded: 好友添加\n")
            append("• onFriendRemoved: 好友删除\n\n")

            append("触发回调的操作:\n")
            append("• 更新好友昵称 → 好友信息变更\n")
            append("• 接收他人好友申请 → 好友添加申请\n")
            append("• 删除好友 → 好友删除\n")
            append("• 别人同意你的好友申请 → 好友添加\n")
            append("• 别人拒绝你的好友申请 → 好友申请拒绝\n\n")
