...
import com.netease.nim.samples.databinding.FragmentAddConversationListenerBinding
import com.netease.nim.samples.localconversation.common.GlobalConversationListenerManager
import java.text.SimpleDateFormat
import java.util.*

class AddConversationListenerFragment : BaseMethodExecuteFragment<FragmentAddConversationListenerBinding>() {
    ...
    override fun createViewBinding(
        ...
        container: ViewGroup?
    ): FragmentAddConversationListenerBinding {
        return FragmentAddConversationListenerBinding.inflate(inflater, container, false)
    }
    ...
    private fun addConversationListener() {
        ...
        val success = GlobalConversationListenerManager.addListener(requireContext())
        if (success) {
            showToast("✅ 全局会话监听器添加成功")
            updateUI()
        } else {
            showToast("❌ 添加监听器失败")
        }
    ...
    private fun clearHistory() {
        GlobalConversationListenerManager.clearHistory()
        updateUI()
        showToast("已清空所有历史记录")
    }
    ...
    private fun updateListenerStatus() {
        ...
        // 更新最新回调显示
        val latestCallback = GlobalConversationListenerManager.getLatestCallback()
    ...
    private fun updateStatistics() {
        val statistics = GlobalConversationListenerManager.getStatistics()
        val statisticsText = "总回调次数: ${statistics.totalCallbackCount}\n" +
                "同步事件: ${statistics.syncEventCount} | 会话变更: ${statistics.conversationChangeCount} | 未读数变更: ${statistics.unreadCountChangeCount}"
    ...
    private fun updateHistoryDisplay() {
        val history = GlobalConversationListenerManager.getCallbackHistory()

        if (history.isEmpty()) {
            binding.tvCallbackHistory.text = "暂无历史记录\n\n请先添加全局监听器，然后进行会话相关操作来触发回调事件"
            return
        }
    ...
    private fun updateListenTime() {
        val listenStartTime = GlobalConversationListenerManager.getListenStartTime()
        if (GlobalConversationListenerManager.isListenerAdded() && listenStartTime > 0) {
    ...
    override fun onRequest() {
        ...
        val statusText = buildString {
            append("全局会话监听器状态:\n")
            append("是否已添加: ${if (GlobalConversationListenerManager.isListenerAdded()) "是" else "否"}\n")
            ...
            append("历史记录数量: ${GlobalConversationListenerManager.getCallbackHistory().size}\n\n")
        }
