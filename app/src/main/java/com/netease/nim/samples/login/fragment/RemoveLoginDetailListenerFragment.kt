...
import com.netease.nim.samples.databinding.FragmentRemoveLoginDetailListenerBinding
import com.netease.nim.samples.login.common.GlobalLoginDetailListenerManager

class RemoveLoginDetailListenerFragment : BaseMethodExecuteFragment<FragmentRemoveLoginDetailListenerBinding>() {
    ...
    override fun createViewBinding(
        ...
        container: ViewGroup?
    ): FragmentRemoveLoginDetailListenerBinding {
        return FragmentRemoveLoginDetailListenerBinding.inflate(inflater, container, false)
    }
     ...
     * 移除登录详情监听器
     */
    private fun removeLoginDetailListener() {
        ...
        // 执行移除操作
        val success = GlobalLoginDetailListenerManager.removeListener()

        if (success) {
            updateOperationResult("✅ 监听器移除成功! 已停止接收所有连接状态和数据同步回调")
            showToast("✅ 监听器移除成功")
        } else {
    ...
    override fun onRequest() {
        ...
        val isListenerActive = GlobalLoginDetailListenerManager.isListenerAdded()
        ...
        val statusText = buildString {
            ...
            if (isListenerActive) {
                ...
                append("✅ 发现活跃监听器:\n")
                append("• 类型: V2NIMLoginDetailListener\n")
                append("• 状态: 全局监听中\n")
                append("• 可以执行移除操作\n\n")

                append("执行移除:\n")
                append("• removeLoginDetailListener: 移除指定监听器\n")
